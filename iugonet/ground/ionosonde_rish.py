...
def ionosonde_rish(
    ...
    #===== Set parameters (1) =====#
    file_format = 'txt'
    remote_data_dir = 'http://database.rish.kyoto-u.ac.jp/arch/mudb/ionosonde/data/text/'
    local_path = 'rish/'
    prefix = 'rish_'
    time_format = ['Y', 'm', 'd', 'H', 'M']
    time_column = [1, 2, 3, 4, 5]
    input_time = [-1, -1, -1, -1, -1, 0]
    suffix_hour = ['00', '23'] # ['Start time', 'End time']
    suffix_minutes = ['00', '15', '30', '45']
    file_res = 900.
    data_start = 11
    site_list = ['sgk']
    datatype_list = ['']
    ...
    if (int(suffix_hour[1]) - int(suffix_hour[0])) != 1:
        tmp = []
        for i in range(int(suffix_hour[1]) - int(suffix_hour[0])):
            tmp.append('%02d' % (int(suffix_hour[0]) + i))
        tmp.append('%02d' % (int(suffix_hour[1]) - int(suffix_hour[0])))
        suffix_hour = tmp
    # site
    ...
    for st in st_list:
        ...
        for dt in dt_list:
            ...
            for pr in pr_list:
                ...
                loaded_data_temp = load_ionosonde(trange=trange, site=st, datatype=dt, parameter=pr, \
                                            pathformat=pathformat, file_res=file_res, remote_path=remote_data_dir, \
                                            local_path=local_path, no_update=no_update, downloadonly=downloadonly, \
                                            uname=uname, passwd=passwd, prefix=prefix, suffix=suffix, \
                                            get_support_data=get_support_data, varformat=varformat, varnames=varnames, \
