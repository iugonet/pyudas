    ...
    for st in st_list:
        ...
        for dt in dt_list:
            ...
            for pr in pr_list:
				...
				#===== Set parameters (2) =====#
                pathformat = 'ask/'+st+'/%Y/nipr_ask_'+st+'_'+dt+'_%Y%m%d_v??.cdf'
				#==============================#
                ...
                if (not downloadonly) and (not notplot):
                    ...
                    current_tplot_name = prefix+'keo_raw_ns_'+st+'_'+dt
                    if current_tplot_name in loaded_data:
                        ...
                        if get_data_vars is None:
                            ...
                            #;--- Rename
                            new_tplot_name = prefix+'keo_'+st+'_'+dt+'_ns'
                            ...
                            options(new_tplot_name, 'ytitle', st.upper()+' '+dt+' NS keogram')
                            options(new_tplot_name, 'ysubtitle', '[counts]')

                    current_tplot_name = prefix+'keo_raw_ew_'+st+'_'+dt
                    if current_tplot_name in loaded_data:
                        get_data_vars = get_data(current_tplot_name)
                        if get_data_vars is None:
                            store_data(current_tplot_name, delete=True)
                            ...
                            #;--- Rename
                            new_tplot_name = prefix+'keo_'+st+'_'+dt+'_ew'
                            store_data(current_tplot_name, newname=new_tplot_name)
                            loaded_data.remove(current_tplot_name)
                            loaded_data.append(new_tplot_name)
                            #;--- Ylim
                            ...
                            #;--- Options
                            options(new_tplot_name, 'Spec', 1)
                            options(new_tplot_name, 'ytitle', st.upper()+' '+dt+' EW keogram')
                            options(new_tplot_name, 'ysubtitle', '[counts]')
